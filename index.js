...
function processSum(numberList, callback) {
  return callback (numberList.reduce(function(total, amount){
    total + amount 
 }),0)  
}
...
function getFullNames(runners) {
  ...
  runners.foreach(object => people.push(`${object.last_name}, ${object.first_name}`));
...
function firstNamesAllCaps(runners) {
  let drunkMan = runners.map (first_name.toUpperCase);
  return drunkMan
}
...
function getRunnersByTShirtSize(runners) {
  const findSize = runners.filter(shirts => (shirts.shirt_size) === "S", "M", "L", "XL", "2XL", "3XL" );
  return findSize
}
...
function tallyUpDonations(runners) {
   const findSum = runners.reduce( (total.donation) => total + donation, 0);
   return findSum
}
